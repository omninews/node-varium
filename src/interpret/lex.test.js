...
describe("Lex", () => {
  ...
  it("should parse a default value", () => {
    expect(lex("THING : String | Value")).to.have.deep.property(
      "[2].value",
      "Value"
    );
    expect(lex("THING : String |Value")).to.have.deep.property(
    ...
    expect(lex("THING : String | Ö value")).to.have.deep.property(
      "[2].value",
      "Ö value"
    );
    expect(
      lex("THING : String|http://example.com?mjau ")
    ).to.have.deep.property("[2].value", "http://example.com?mjau");
  });
  ...
  it("should parse a quoted default value", () => {
    ...
    expect(lex('THING : String | "Value # hash"')).to.have.deep.property(
      "[2].value",
      "Value # hash"
    );
    expect(lex('THING : String | "Value\nnewline"')).to.have.deep.property(
      "[2].value",
      "Value\nnewline"
    );
    expect(
      lex('THING : String | "Value\nnewline" #A comment')
    ).to.have.deep.property("[2].value", "Value\nnewline");
    expect(lex('THING : String | " spaces "')).to.have.deep.property(
      "[2].value",
      " spaces "
    );
  });
  ...
  it("should parse escaped chars in a quoted default value", () => {
    expect(lex('THING : String | "Value\\"quote"')).to.have.deep.property(
      "[2].value",
      'Value"quote'
    );
    expect(lex('THING : String | "Backslash: \\\\"')).to.have.deep.property(
      "[2].value",
      "Backslash: \\"
    );
  });
  ...
  it("should parse a comment", () => {
    ...
    expect(() =>
      lex("THING : String | Value#Anöthing goes|in a comment!#")
    ).not.to.throw();
  ...
  it("should parse multiple declarations", () => {
    ...
    lex(
      "ONE : String | Default # Comment \n\nTWO : String | #Comment\n"
    ).should.eql(res2);
  });

  it("should fail on invalid name", () => {
    expect(() => lex("THINGö : String")).to.throw(
      "Unexpected 'ö' in DeclarationName"
    );
